...
module.exports = class AsciiDocParser {
  ...
  readBlock (dataLines, opts = {}) {
    ...
    if (nextLine) {
      ...
      else if (this.isAttributeEntry(nextLine)){
        block = this.readAttributeEntry(dataLines);
      }
      else {
  ...
  readAttributeEntry(dataLines) {
    const attributeLine = this.readLine()
    return {
      type: 'AttributeEntryNode',
      data: dataLines,
      raw: attributeLine.rawText,
      range: attributeLine.range,
      // QUESTION should we include dataLines when defining start?
      loc: attributeLine.loc
    }
  }
  ...
  isAttributeEntry (line) {
    ...
    return AttributeEntryRx.test(lineText)
  }
